	...
	public void run(){
		try{
			PreparedStatement statement = DecimateNetworkCore.getInstance().getConnection().prepareStatement(
					"SELECT * FROM `NetworkPunishments`"
			);
			...
			while(resultSet.next()){
				...
				DecimateNetworkCore.getInstance().getPunishmentManager().loadPunishment(new Punishment(id, type, reason, applied, expiration, punisherUuid, punishedUuid, reverted));
			}
